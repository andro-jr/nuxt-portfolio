<template>
  <div class="not-prose">
    <p class="mb-10">Take a look at my Github Projects.</p>

    <section v-if="pending">Loading...</section>
    <section v-else-if="error">Error</section>
    <section v-else>
      <ul class="grid grid-cols-1 gap-4">
        <li
          v-for="repository in repos"
          :key="repository.id"
          class="border border-gray-200 rounded-sm hover:bg-gray-100 dark:hover:bg-gray-800 font-mono"
        >
          <a
            :href="repository.html_url"
            target="_blank"
            class="cursor-pointer inline-block p-4 w-full"
          >
            <div class="flex items-center justify-between text-sm">
              <div class="font-semibold">{{ repository.name }}</div>
              <div>{{ repository.stargazers_count }} ★</div>
            </div>
            <p class="text-sm">{{ repository.description }}</p>
          </a>
        </li>
      </ul>
    </section>
  </div>
</template>

<script setup>
const { error, pending, data } = await useFetch(
  "https://api.github.com/users/andro-jr/repos"
);

const repos = computed(() =>
  data.value.sort((a, b) => b.stargazers_count - a.stargazers_count)
);
</script>
